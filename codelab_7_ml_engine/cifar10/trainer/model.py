...
tf.app.flags.DEFINE_boolean('use-fp16', False,
...
def inputs(eval_data):
  ...
  if not FLAGS.data_dir:
    raise ValueError('Please supply a data_dir')
  data_dir = os.path.join(FLAGS.data_dir, 'cifar-10-batches-bin')
  images, labels = input.inputs(eval_data=eval_data, data_dir=data_dir,
                                batch_size=FLAGS.batch_size)
  if FLAGS.use_fp16:
    images = tf.cast(images, tf.float16)
    labels = tf.cast(labels, tf.float16)
...
def inference(images):
  ...
  with tf.variable_scope('local3') as scope:
    ...
    reshape = tf.reshape(pool2, [-1, RESHAPE_SIZE])
...
def build_prediction_graph():
  ...
  image = tf.map_fn(decode_and_resize, image_bytes, back_prop=False,
                    dtype=tf.float32)
